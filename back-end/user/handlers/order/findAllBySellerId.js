...
const handler = HandlerWithResponse(
  async ({ user, params: { userId }, query: { page, limit } }) => {
    ...
    const orders = await Order.findAllBySellerId(userId, pagination)
...
module.exports = {
  middlewares: [requireAuth, validate(validationSchema)],
  handler,
  method: 'get',
  path: '/users/:userId/seller/orders',
}
